...
CREATE PROCEDURE new_order_2 (in_w_id INT,
	                      ...
	                      in_ol_i_id INT,
	                      in_ol_quantity INT,
	                      in_i_price NUMERIC(5, 2),
	                      in_i_name TEXT,
	                      in_i_data TEXT,
	                      in_ol_o_id INT,
	                      in_ol_amount NUMERIC(10, 2),
...
DECLARE	tmp_brand_generic CHAR(1);
	...
	IF LOCATE('ORIGINAL', in_i_data) > 0
	   AND LOCATE('ORIGINAL', tmp_s_data) > 0 THEN
		SET tmp_brand_generic = 'B';
	ELSE
		SET tmp_brand_generic = 'G';
	END IF;
	...
	IF out_s_quantity >= in_ol_quantity + 10 THEN
		...
		SET s_quantity = s_quantity - in_ol_quantity,
		    ...
		    s_remote_cnt = s_remote_cnt
		                 + IF(in_ol_supply_w_id <> in_w_id, 1, 0)
		WHERE s_i_id = in_ol_i_id
		  AND s_w_id = in_ol_supply_w_id;
	ELSE
		UPDATE stock
