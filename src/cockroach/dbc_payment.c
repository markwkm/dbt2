...
int execute_payment_cockroach(
		struct db_context_t *dbc, struct payment_t *data) {
	PGresult *res;
	...
	paramValues[1] = w_id;
	res = PQexecParams(
			dbc->library.libpq.conn, PAYMENT_1, 2, NULL, paramValues, NULL,
			NULL, 0);
	...
	for (i = 0; i < PQntuples(res); i++) {
		LOG_ERROR_MESSAGE(
				...
				PQgetvalue(res, i, 2), i, PQgetvalue(res, i, 3), i,
				PQgetvalue(res, i, 4), i, PQgetvalue(res, i, 5));
	}
#endif /* DEBUG */
	PQclear(res);
	...
	paramValues[2] = w_id;
	res = PQexecParams(
			dbc->library.libpq.conn, PAYMENT_2, 3, NULL, paramValues, NULL,
			NULL, 0);
	if (!res || PQresultStatus(res) != PGRES_TUPLES_OK) {
		LOG_ERROR_MESSAGE("%s", PQerrorMessage(dbc->library.libpq.conn));
		PQclear(res);
	...
	if (data->c_id == 0) {
		...
		res = PQexecParams(
				dbc->library.libpq.conn, PAYMENT_3, 3, NULL, paramValues, NULL,
				NULL, 0);
		if (!res || PQresultStatus(res) != PGRES_TUPLES_OK) {
			LOG_ERROR_MESSAGE("%s", PQerrorMessage(dbc->library.libpq.conn));
			PQclear(res);
	...
	paramValues[2] = c_id;
	res = PQexecParams(
			dbc->library.libpq.conn, PAYMENT_4, 3, NULL, paramValues, NULL,
			NULL, 0);
	if (!res || PQresultStatus(res) != PGRES_TUPLES_OK) {
		LOG_ERROR_MESSAGE("%s", PQerrorMessage(dbc->library.libpq.conn));
		PQclear(res);
	...
	for (i = 0; i < PQntuples(res); i++) {
		LOG_ERROR_MESSAGE(
				...
				i, PQgetvalue(res, i, 0), i, PQgetvalue(res, i, 1), i,
				PQgetvalue(res, i, 2), i, PQgetvalue(res, i, 3), i,
				PQgetvalue(res, i, 4), i, PQgetvalue(res, i, 5), i,
				PQgetvalue(res, i, 6), i, PQgetvalue(res, i, 7), i,
				PQgetvalue(res, i, 8), i, PQgetvalue(res, i, 9), i,
				PQgetvalue(res, i, 10), i, PQgetvalue(res, i, 11), i,
				PQgetvalue(res, i, 12), i, PQgetvalue(res, i, 13));
	}
#endif /* DEBUG */
	PQclear(res);
	...
	if (gc) {
		...
		res = PQexecParams(
				dbc->library.libpq.conn, PAYMENT_5_GC, 4, NULL, paramValues,
				NULL, NULL, 0);
		if (!res || PQresultStatus(res) != PGRES_COMMAND_OK) {
			LOG_ERROR_MESSAGE(
					"P5GC Error: %s\n"
					...
					"P5GC c_w_id = %s\n"
					"P5GC c_d_id = %s",
					PQerrorMessage(dbc->library.libpq.conn), PAYMENT_5_GC,
					h_amount, c_id, c_w_id, c_d_id);
			PQclear(res);
			return ERROR;
		}
		c_data[0] = '\0';
		...
		res = PQexecParams(
				dbc->library.libpq.conn, PAYMENT_5_BC, 5, NULL, paramValues,
				NULL, NULL, 0);
		if (!res || PQresultStatus(res) != PGRES_TUPLES_OK) {
			LOG_ERROR_MESSAGE("%s", PQerrorMessage(dbc->library.libpq.conn));
			PQclear(res);
		...
		if (PQntuples(res) == 0) {
			LOG_ERROR_MESSAGE(
					...
					"P5BC c_w_id = %s\n"
					"P5BC c_d_id = %s",
					PAYMENT_5_BC, h_amount, c_data, c_id, c_w_id, c_d_id);
			PQclear(res);
			return ERROR;
		}
	...
	res = PQexecParams(
			dbc->library.libpq.conn, PAYMENT_6, 8, NULL, paramValues, NULL,
			NULL, 0);
	if (!res || PQresultStatus(res) != PGRES_TUPLES_OK) {
		LOG_ERROR_MESSAGE("%s", PQerrorMessage(dbc->library.libpq.conn));
		PQclear(res);
