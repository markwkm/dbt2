...
int commit_transaction_sqlite(struct db_context_t *dbc) {
  if (dbc->library.sqlite.inTransaction) {
    if (sqlite3_exec(dbc->library.sqlite.db, "COMMIT TRANSACTION;", NULL, NULL,
                     NULL) != SQLITE_OK) {
      LOG_ERROR_MESSAGE("COMMIT TRANSACTION;\nsqlite reports: %d %s",
                        sqlite3_errcode(dbc->library.sqlite.db),
                        sqlite3_errmsg(dbc->library.sqlite.db));
      return ERROR;
    }
...
int rollback_transaction_sqlite(struct db_context_t *dbc) {
  ...
  if (dbc->library.sqlite.inTransaction) {
    if (sqlite3_exec(dbc->library.sqlite.db, "ROLLBACK TRANSACTION;", NULL,
                     NULL, NULL) != SQLITE_OK) {
...
int sql_execute_sqlite(struct db_context_t *dbc, char *query,
  ...
  if (!dbc->library.sqlite.inTransaction) {
    if (sqlite3_exec(dbc->library.sqlite.db, "BEGIN TRANSACTION;", NULL, NULL,
                     NULL) != SQLITE_OK) {
      LOG_ERROR_MESSAGE("%s: BEGIN TRANSACTION;\nsqlite reports: %d %s",
                        query_name, sqlite3_errcode(dbc->library.sqlite.db),
                        sqlite3_errmsg(dbc->library.sqlite.db));
      return 0;
    }
  ...
  if (sqlite3_prepare_v2(dbc->library.sqlite.db, query, strlen(query) + 1,
                         &sql_result->library.sqlite.pStmt,
                         NULL) != SQLITE_OK) {
  ...
  sql_result->library.sqlite.num_fields =
      sqlite3_column_count(sql_result->library.sqlite.pStmt);
...
int sql_fetchrow_sqlite(struct db_context_t *dbc,
...
char *sql_getvalue_sqlite(struct db_context_t *dbc,
  ...
  tmp = NULL;
  if (sql_result->library.sqlite.query_running &&
      field < sql_result->library.sqlite.num_fields) {
    tmp = strndup(
        (char *)sqlite3_column_text(sql_result->library.sqlite.pStmt, field),
        sqlite3_column_bytes(sql_result->library.sqlite.pStmt, field));
    if (!tmp) {
#ifdef DEBUG_QUERY
...
#ifdef DEBUG_QUERY
    LOG_ERROR_MESSAGE("dbt2_sql_getvalue: POSSIBLE NULL VALUE or "
                      "ERROR\nQuery: %s\nField: %d from %d",
                      sqlite3_sql(sql_result->library.sqlite.pStmt), field,
                      sql_result->library.sqlite.num_fields);
#endif
  }
