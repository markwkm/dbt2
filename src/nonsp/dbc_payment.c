...
int payment_nonsp(
		struct db_context_t *dbc, struct payment_t *data, char **vals,
		int nvals) {
	/* Input variables. */
	int w_id = data->w_id;
	int d_id = data->d_id;
...
#endif
	if ((*dbc->sql_execute)(dbc, query, &result, "PAYMENT_1") &&
	...
	if ((*dbc->sql_execute)(dbc, query, &result, "PAYMENT_3") &&
		result.library.sqlite.query_running) {
		(*dbc->sql_fetchrow)(dbc, &result);
...
#endif
	if ((*dbc->sql_execute)(dbc, query, &result, "PAYMENT_6") &&
		result.library.sqlite.query_running) {
		(*dbc->sql_fetchrow)(dbc, &result);
		...
		if (!vals[C_CREDIT]) {
			LOG_ERROR_MESSAGE(
					"ERROR: C_CREDIT=NULL for query PAYMENT_6:\n%s\n", query);
			return -1;
		}
	} else // error
	{
	...
	sprintf(query, PAYMENT_8, my_c_id, c_d_id, c_w_id, d_id, w_id, h_amount,
			my_w_name, my_d_name);
